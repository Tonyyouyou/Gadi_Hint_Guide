...
- [`skills/gadi-autoresearch`](skills/gadi-autoresearch/SKILL.md): a bounded, resumable broad-idea-to-paper research lab using a single-writer Director, blind opportunity scouts and evidence analysts, staged hypothesis maturity, independent review, persistent control, PBS experiments, and compact inode-safe state.
...
- Autoresearch keeps hypothesis evolution in only `RESEARCH_GRAPH.json` and `LEARNING_LEDGER.jsonl`; completed evidence-bearing batches receive blind analysis, every terminal experiment is interpreted, critics advise only material changes, and the Director records bounded decisions.
